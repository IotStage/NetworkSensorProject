...
#define SensorPinTemperature 55
...
void loop() {
  ...
  if(sx1276.availableData(100))
  {
    ...
    Serial.println(len);
    //Traitement des messages de commande
...
String getTrame(){
  /*frame_data.ph = Ph.getMesure();
  frame_data.ec = EC.getMesure();
  frame_data.orp = ORP.getMesure();
  frame_data.tur = Turbidity.getMesure();
  frame_data.ane = getAnemometreMesure();
  frame_data.gir = girouette.getMesure();
  frame_data.pl = (float)nbBasculement;
  frame_data.t1 = temp;
  */
  String trame="";
  ...
  trame+=String((temp), 1);
  trame+=";";
  trame+=String(getAnemometreMesure(), 1);
  trame+=";";
...
void sendPaquet(String paquet, int idClient){
  ...
  sx1276.sendPacketTimeout(idClient, buff2);
